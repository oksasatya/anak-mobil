...
async fn the_cost_matrix_holds_for_every_setting_and_both_viewers() {
    ...
    for (setting, car) in &cars {
        ...
        let cost = &build["modifications"][0]["cost"];
        if *setting == "private" {
...
| Delete the `from_role == to_role` branch | `setting_the_role_somebody_already_has_writes_nothing_and_is_not_an_error` (it becomes a `Database` error from the `CHECK`) |
...
1. Running it a **second time** for the same address prints the `no change` line and exits 0 — `set_role` returns `Ok(None)` because the account already has the role, so the bootstrap precondition is never reached. Confirm no second audit row: `SELECT count(*) FROM role_changes`.
...
- **`grant-admin` run twice for the same address hits the no-op branch, not the bootstrap precondition**, because `set_role` re-reads the target's role before it reaches any of that. It prints "no change" and exits 0. That is the right behaviour and it is worth knowing before somebody reads it as a bug.
...
| 3 `PlatformRole`, the `Admin` extractor, the admin vehicle read | not started | |
...
Written the moment a finding arrives, not at the end — chat is lost to compaction and a deferred finding nobody can find again is a finding dropped rather than deferred.

| Task | Severity | File and line | Failure scenario | Smallest fix | Status |
|---|---|---|---|---|---|

Severity vocabulary: `structural` (a column, a constraint, a public contract, or an authorization boundary — raise and fix **immediately**, because later tasks build on it) · `security` · `correctness` · `test-integrity` · `hygiene`. Everything but `structural` and `security` is worked in one fix pass after the final task.

**Three specific things a reviewer of this ticket should hunt**, beyond the usual:

1. **An assertion that cannot fail.** AM-361's reviewers found nine of twelve range guards, nine of eleven enum mappings, and five mutations of the cost filter surviving a green suite. Every sabotage table in this plan names which test must die; run them.
2. **A role produced by anything other than the database.** An `unwrap_or`, a default, a value cached on `AppState`, a `_` match arm.
3. **A comment describing a mechanism that moved.** Three separate AM-361 findings were a correct comment about code that no longer worked that way, and each one sent the next reader to the wrong place.
