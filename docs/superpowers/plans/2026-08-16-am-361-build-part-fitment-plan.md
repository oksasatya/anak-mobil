...
CREATE TABLE part_merges (
    ...
    -- NOT an equivalence. `(undone_at IS NULL) = (undone_by IS NULL)`
    -- contradicts `undone_by ... ON DELETE SET NULL` two lines above: deleting
    -- a curator who once undid a merge writes undone_by = NULL while
    -- undone_at stays set, the CHECK rejects it, and the account deletion
    -- fails outright — exactly backwards from why SET NULL was chosen.
    -- Proven against the live schema. Do not "tidy" this back.
);
...
| 3.1 Three-query list and batched counts | 3 | **landed** | Found and fixed a privacy leak in the plan's own SQL: `COUNT(DISTINCT b.id)` ignored the visibility filter entirely — 1350 counted where 32 was correct. |
...
| 4.1 `part_merges` | 4 | **landed** | Reviewer found the plan's own CHECK made account deletion impossible — the equivalence contradicted `ON DELETE SET NULL`. Corrected in the plan before 4.2 read it. |
...
| Fix pass | — | **landed** | Five ledger findings closed. Two merged migrations were briefly amended and reverted — `sqlx migrate` refused them, this repository's own four-condition rule failing exactly as written, against the person who wrote it. |
| Loud-skip completion | — | **landed** | The fix pass closed one guard of four and reported the finding closed. Within the hour the suite reported fifteen green boards with no database. All four are loud now. |

**Two tasks did not get the independent review this loop promises, and no summary should imply otherwise.** Task 2.1's reviewer was never dispatched — the controller was diverted and did not come back to it. Task 4.2's reviewer stopped without producing a verdict; that is the hardest task in the plan, and its defects were found by its own writer rather than by an independent read.
...
| 4.1 | **`test-integrity` — plan text was the hazard, FIXED** | Plan line 4088 | The plan still carried `CHECK ((undone_at IS NULL) = (undone_by IS NULL))` — the equivalence that makes account deletion fail outright when a curator who once undid a merge closes their account. It reads more naturally than the one-way form, so somebody "tidying" would restore it, and **nothing would go red**. Third instance of "plan text still stale" in this ticket and the only one whose stale text is a hard production failure rather than a slow query. | Plan corrected in place with the reasoning and a "do not tidy this back" line. The regression test is assigned into 4.2's brief — the analogue is `catalog_flow.rs:412`. | **closed in plan; test assigned to 4.2** |
| 4.1 | `correctness` | `part_merges_undo_complete` | The one-way CHECK no longer forces an undo to name its actor: `UPDATE … SET undone_at = now()` with no `undone_by` is accepted, verified. And such a row is byte-identical to one where a real curator undid the merge then closed their account — so "undone by somebody since departed" and "undone with no actor recorded" are indistinguishable. Accountability is not merely unanswerable, it is *ambiguously* unanswerable. The equivalence bought a write-time guarantee and cost a deletion-time failure; only the second was tested. | Bind `undone_by` explicitly in 4.2's undo and pin it with a test. A `BEFORE UPDATE` trigger on the `undone_at NULL → non-NULL` transition would enforce it at schema level and close the append-only gap too — worth having when an admin surface exists. | **assigned into 4.2's brief** |
| 4.1 | `hygiene` | The table comment, and the plan's Big O | "Append-only" is a comment, not a constraint — `UPDATE` rewriting `source_part_id` and `DELETE` removing history both verified working. And the plan's Big O names `part_merges_source_idx`, which **does not exist**; the recompute reads the whole live log with no predicate on either column, so no index can narrow it. A wrong performance comment is what makes a future profiler look elsewhere — the same diagnosis as the 1.4 entry. | Say the discipline is enforced by the use case and name what would enforce it later. Write `O(M)` over live merges and the real reason. | **closed — fix pass**, recorded in `apps/api/CLAUDE.md` rather than the frozen migration |
| 3.1 | **`security` — FIXED IN TASK** | The plan's drafted `evidence_counts` SQL | `COUNT(DISTINCT b.id)` where `builds` joins **unconditionally** and the visibility+variant predicate sits in the `ON` clause of the `vehicles` join beneath it. `b.id` is therefore non-NULL regardless, so the filter was ignored and every build referencing the part was counted — **private builds belonging to other people, surfaced through a statistic.** Measured at 50k builds, 98% private, viewer owning none: **1350 where 32 is correct.** No compile error, no obviously failing test; it would have shipped. | `COUNT(DISTINCT v.id)` — `v` only joins when the predicate holds and `COUNT` ignores NULLs. `builds.vehicle_id` is UNIQUE so the two count the same set, gated this time. **The plan text still carries the wrong SQL and must not be copied by a later reader.** | **closed in task; plan text still stale** |
...
| 1.5 | `security` | `usecase/parts.rs:129-137` — the daily allowance | Check-then-act: `suggested_since` and the insert are two autocommit statements with nothing serialising them. N concurrent POSTs all read the same count and all pass; the ceiling is the connection pool, not `PARTS_PER_DAY = 20`. **The identical defect is in `usecase/catalog.rs:85-89`, so it is a pattern, not a slip.** Cost is bounded — the threat is denial of service against curation, not money, and the queue is human-read. | Either `pg_advisory_xact_lock(hashtext(user_id))` around count-and-insert in **both** places, or a `ponytail:` comment naming the ceiling in both. Not one without the other. | **closed — fix pass.** It needed a second fix to be real: `suggest` held a pooled autocommit connection, and a transaction-scoped advisory lock there is taken and released before the next statement — a guard that looks right and does nothing. |
| 1.5 | `correctness` | `check()` — no length bound on `brand` / `product_name` | `parts_identity` is a btree over two unbounded `TEXT` columns, and btree tuples cap at 2704 bytes. A 6400-character brand gives `ERROR: index row size 6432 exceeds btree version 4 maximum` → 500. The catalog analogue is not exposed: `catalog_suggestions` has only a non-unique index. Bounded by auth and the 20/day allowance. | Length bounds in `check()` beside the emptiness check — `brand` ≤ 120, `product_name` ≤ 200, same Indonesian message shape. | **closed — fix pass.** The first reproduction used compressible text and *passed*, because it TOASTed below the limit; incompressible text is what fails. Nearly concluded wrongly. |
| 1.5 | `hygiene` | The 429 message | *"Terlalu banyak permintaan. Tunggu sebentar."* for a wait of up to 24 hours, with no `Retry-After`. Someone who spent today's allowance retries into the same wall for hours. `shared/i18n.rs:82-87` reserves namespaced domain codes for exactly this. Same wording problem in catalog. | A `parts.daily_limit` code with its own message. | **closed — fix pass** |
